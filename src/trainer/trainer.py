...
class Trainer:
    ...
    def __init__(self, model: torch.nn.Module, dataloader: tuple, config: Config) -> None:
        ...
        if config.summary.add_sample_batch:
            add_input_samples(dataloader=train_loader, writer=self.writer, tag="train", global_step=0)
            add_input_samples(dataloader=test_loader, writer=self.writer, tag="test", global_step=0)
